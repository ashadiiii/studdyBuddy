...
mcp = FastMCP("semanticscholar", timeout=30)  # 15 second timeout

# Initialize SemanticScholar client
client = initialize_client()

@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
async def retry_search_papers(*args, **kwargs):
    return await asyncio.to_thread(search_papers, *args, **kwargs)
...
async def search_semantic_scholar(query: str, num_results: int = 10) -> List[Dict[str, Any]]:
    ...
    """
    try:
        results = await retry_search_papers(client, query, num_results)
        return results
    except Exception as e:
        logging.error(f"Error in search_semantic_scholar: {str(e)}")
